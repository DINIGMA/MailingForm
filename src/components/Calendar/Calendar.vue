...
const checkDate = (day) => {
  ...
  if (foundDate) {
    ...
    fillPercentage = Math.ceil(
      (getDates.value[`${foundDate}`].engaged_count / getDefualtDayCapacity.value) * 100
    )
...
<template>
  <div class="calendar">
    <div class="calndar__close" @click="emits('close-popup')"></div>
    <h1 class="calendar__title">Задание на WhatsApp</h1>
    ...
    <div class="calendar__content">
      <div class="calendar__header">
        ...
        <div class="calendar__toggle-date">
          <div class="toggle-item toggle-day">
            ...
            />
            <label for="month-btn">Месяц</label>
          </div>
        ...
        <ul class="calendar__dates">
          <li v-for="(week, index) in calendar" :key="index" class="calendar__weeks">
            ...
            >
              <span>{{ day.day }}</span>
              <div
                v-if="checkDate(day.date) && !isPastDate(day.date)"
                class="calendar__progress-bar"
                :style="{ height: `${checkDate(day.date)}%` }"
              ></div>
            </div>
          </li>
        </ul>
      ...
      <div v-else>
        <div class="calendar__info">
          <span
            >Свободно на день <b>{{ getCalendarInfo }}</b> из
            <b>{{ getDefualtDayCapacity }}</b></span
          >
        </div>
        ...
        <button v-if="selectedTime" @click="emits('close-popup')" class="complete">
          <span>Готово</span>
        </button>
      </div>
    </div>
